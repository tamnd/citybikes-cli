...
describes what users get. Pick a format with `-o`, or let citybikes choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
citybikes <command> -o table   # aligned columns for reading
citybikes <command> -o jsonl   # one JSON object per line, for piping
citybikes <command> -o json    # a single JSON array
citybikes <command> -o csv     # spreadsheet friendly
citybikes <command> -o tsv     # tab-separated
citybikes <command> -o url     # just the URL column
citybikes <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
citybikes <command> --fields id,title,url
```
...
```bash
citybikes <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
citybikes <command>            # a table, because this is a terminal
citybikes <command> | wc -l    # JSONL, because this is a pipe
```
